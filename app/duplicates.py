"""
This file contains intentional duplicate code for SonarQube demonstration
Total lines: 500+
Purpose: Testing duplicate code detection and analysis
"""
...
# Section 1: User Management Duplicates
class UserService:
...
class UserManager:
    def add_user(self, name: str, email: str, age: int):
        """Add a new user with validation - DUPLICATE"""
        if not name or len(name) < 2:
            raise ValueError("Name must be at least 2 characters")
        if not email or "@" not in email:
            raise ValueError("Invalid email format")
...
class UserHandler:
    def register_user(self, name: str, email: str, age: int):
        """Register a new user with validation - DUPLICATE"""
        if not name or len(name) < 2:
            raise ValueError("Name must be at least 2 characters")
        if not email or "@" not in email:
            raise ValueError("Invalid email format")
...
# Section 2: Email Validation Duplicates
...
def check_email_format(email: str) -> bool:
    """Check email format with comprehensive validation - DUPLICATE"""
    if not email or not isinstance(email, str):
        return False
...
def is_valid_email(email: str) -> bool:
    """Check if email is valid with comprehensive validation - DUPLICATE"""
    if not email or not isinstance(email, str):
        return False
...
# Section 3: Password Validation Duplicates
...
def check_password_strength(password: str) -> Dict[str, Any]:
    """Check password strength with detailed feedback - DUPLICATE"""
    result = {
        "valid": False,
        "strength": "weak",
        "errors": [],
...
def is_password_valid(password: str) -> Dict[str, Any]:
    """Validate password with detailed feedback - DUPLICATE"""
    result = {
        "valid": False,
        "strength": "weak",
        "errors": [],
...
# Section 4: Data Processing Duplicates
class DataProcessor:
...
class DataHandler:
    def handle_data(self, data: List[Dict]):
        """Handle raw data and return formatted results - DUPLICATE"""
        if not data:
            return []
...
class DataManager:
    def manage_data(self, data: List[Dict]):
        """Manage raw data and return formatted results - DUPLICATE"""
        if not data:
            return []
...
# Section 5: API Response Formatting
...
def create_api_response(data: Any, message: str = "Success", status_code: int = 200) -> Dict:
    """Create API response with consistent structure - DUPLICATE"""
    response = {
        "success": status_code < 400,
        "status_code": status_code,
        "message": message,
